...
float clamp(float r,float min, float max)
{
    if(r<min) return min;
    if(r>max) return max;
    return r;
...
struct Camera
{
    ...
    Camera(const Point3D& pos, const Vec3f& lookat, const Vec3f& up): position{pos},lookat{lookat},updirection{up}
    {
        lookdirection=lookat-position.data;//-z
